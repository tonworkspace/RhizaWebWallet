...
const GlobalNetworkModal: React.FC = () => {
  const { isNetworkModalOpen, setIsNetworkModalOpen, switchEvmChain, currentEvmChain } = useWallet();
  ...
  const handleNetworkSelect = async (chainKey: EvmChain) => {
    ...
    // Switch chain globally via WalletContext
    await switchEvmChain(chainKey);
          ...
          {(Object.keys(CHAIN_META) as EvmChain[]).map((chainKey) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between p-4 rounded-2xl border transition-all ${evmChain === chainKey
