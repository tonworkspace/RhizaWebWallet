...
const BalanceVerification: React.FC = () => {
  ...
  // Don't render anything if address is missing
  ...
  return (
    <div className="space-y-3 pb-4">
      ...
      {/* Header Card (Compact) */}
      <div className={`p-3 sm:p-4 rounded-xl border flex flex-col gap-2.5 sm:gap-3 transition-colors ${isComplete
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
            <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center flex-shrink-0 ${isComplete ? 'bg-emerald-100 dark:bg-emerald-500/20 text-emerald-600 dark:text-emerald-400'
              : 'bg-amber-100 dark:bg-amber-500/20 text-amber-600 dark:text-amber-400'
              }`}>
              {isComplete ? <ShieldCheck size={18} className="sm:w-5 sm:h-5" /> : <Lock size={18} className="sm:w-5 sm:h-5" />}
            </div>
            <div className="min-w-0 flex-1">
              ...
              <p className="text-[10px] sm:text-xs text-slate-600 dark:text-gray-400 truncate">
                {isComplete ? 'RZC transfers are enabled' : 'RZC transfers temporarily locked'}
              </p>
            </div>
          </div>
          <button onClick={handleRefresh} disabled={refreshing}
            className={`p-1.5 rounded-lg transition-colors flex-shrink-0 ${isComplete ? 'hover:bg-emerald-200 dark:hover:bg-emerald-500/30 text-emerald-600'
        ...
        {/* Compact Balance */}
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-0 py-2 sm:py-2.5 px-2.5 sm:px-3 bg-white/60 dark:bg-black/20 rounded-lg border border-slate-200/50 dark:border-white/5">
          <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
            <div>
              <span className="text-[10px] sm:text-[11px] text-slate-500 dark:text-gray-400 font-medium block">Balance</span>
              <span className="text-xs sm:text-sm font-numbers font-bold text-slate-900 dark:text-white">{rzcBalance.toLocaleString()} RZC</span>
            </div>
            <div className="h-6 w-px bg-slate-200 dark:bg-white/10" />
            <div>
              <span className="text-[10px] sm:text-[11px] text-slate-500 dark:text-gray-400 font-medium block">Value</span>
              <span className="text-xs sm:text-sm font-numbers font-bold text-emerald-600 dark:text-emerald-400">${rzcUsd.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
            </div>
          </div>

          {/* Progress Indicators */}
          {!isComplete && (
            <div className="flex items-center gap-1.5 self-end sm:self-auto" title={`${STEPS[currentStepIndex]?.label}`}>
              {STEPS.map((step, idx) => {
        ...
        {!isComplete ? (
          <div className="mt-1 space-y-2">
            {!hasRequest && (
              <div className="flex items-start gap-1.5 sm:gap-2 p-2 sm:p-2.5 rounded-lg bg-amber-50 dark:bg-amber-500/10 border border-amber-100 dark:border-amber-500/20">
                <AlertCircle size={13} className="sm:w-3.5 sm:h-3.5 text-amber-600 dark:text-amber-400 flex-shrink-0 mt-0.5" />
                <div className="min-w-0 flex-1">
                  <p className="text-[9px] sm:text-[10px] font-bold text-amber-900 dark:text-amber-300 mb-0.5">Security Assurance & Verification</p>
                  <p className="text-[8px] sm:text-[9px] text-amber-800 dark:text-amber-400 leading-snug">
            ...
            {hasRequest && verificationRequest ? (
              <div className="py-2 sm:py-2.5 px-2.5 sm:px-3 bg-blue-50/50 dark:bg-blue-500/5 rounded-lg border border-blue-100 dark:border-blue-500/20">
                <div className="flex items-center justify-between mb-1.5 gap-2">
                  <div className="flex items-center gap-1.5 sm:gap-2 min-w-0 flex-1">
                    <FileText size={13} className="sm:w-3.5 sm:h-3.5 text-blue-600 dark:text-blue-400 flex-shrink-0" />
                    <span className="text-[10px] sm:text-xs font-bold text-blue-900 dark:text-blue-300 truncate">Request Submitted</span>
                  </div>
                  <span className={`text-[8px] sm:text-[9px] font-bold px-1.5 py-0.5 rounded uppercase flex-shrink-0 ${verificationRequest.status === 'pending' ? 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400' :
                ...
                <div className="text-[9px] sm:text-[10px] text-blue-700 dark:text-blue-400 flex items-center justify-between gap-2">
                  <span className="truncate">ID: <span className="font-numbers">{verificationRequest.id.slice(0, 8)}</span></span>
                  <span className="flex-shrink-0">Claim: <span className="font-numbers">{verificationRequest.claimed_balance.toLocaleString()}</span></span>
                </div>
              </div>
            ) : (
              <button
                onClick={() => openForm(address)}
                className="w-full py-2 sm:py-2.5 rounded-lg bg-blue-600 hover:bg-blue-700 active:bg-blue-800 text-white font-bold text-[10px] sm:text-xs transition-colors flex items-center justify-center gap-1.5 sm:gap-2 shadow-sm"
              >
          ...
          <div className="mt-1">
            <div className="flex items-start gap-1.5 sm:gap-2 p-2 sm:p-2.5 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-100 dark:border-emerald-500/20">
              <ShieldCheck size={13} className="sm:w-3.5 sm:h-3.5 text-emerald-600 dark:text-emerald-400 flex-shrink-0 mt-0.5" />
              <div className="min-w-0 flex-1">
                <p className="text-[9px] sm:text-[10px] font-bold text-emerald-900 dark:text-emerald-300 mb-0.5">100% Secure & Verified</p>
                <p className="text-[8px] sm:text-[9px] text-emerald-800 dark:text-emerald-400 leading-snug">
      ...
      <div className="rounded-2xl bg-white dark:bg-[#0a0a0a] border border-slate-200 dark:border-white/5 overflow-hidden shadow-sm">
        <button
          ...
          className="w-full flex items-center justify-between px-3 sm:px-5 py-3 sm:py-3.5 hover:bg-slate-50 dark:hover:bg-white/5 active:bg-slate-100 dark:active:bg-white/10 transition-colors"
        >
          <div className="flex items-center gap-2 sm:gap-2.5 min-w-0 flex-1">
            <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-lg bg-purple-50 dark:bg-purple-500/10 flex items-center justify-center flex-shrink-0">
              <Zap size={12} className="sm:w-3.5 sm:h-3.5 text-purple-600 dark:text-purple-400" />
            </div>
            <div className="text-left min-w-0 flex-1">
              <p className="text-[10px] sm:text-[11px] font-black text-slate-900 dark:text-white truncate">RZC Transaction History</p>
              {!txLoading && (
                <p className="text-[8px] sm:text-[9px] text-slate-400 dark:text-gray-600 font-medium">{txHistory.length} records loaded</p>
              )}
            </div>
          </div>
          <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
            {txLoading && <Loader2 size={12} className="sm:w-3.5 sm:h-3.5 text-slate-400 animate-spin" />}
            {expanded ? <ChevronUp size={14} className="sm:w-4 sm:h-4 text-slate-400 dark:text-gray-600" /> : <ChevronDown size={14} className="sm:w-4 sm:h-4 text-slate-400 dark:text-gray-600" />}
          </div>
        </button>

        {expanded && (
          <div className="border-t border-slate-100 dark:border-white/5 px-3 sm:px-5 py-1 max-h-60 sm:max-h-72 overflow-y-auto">
            {txLoading ? (
              <div className="flex items-center justify-center py-6 sm:py-8 gap-2 text-slate-400 dark:text-gray-600">
                <Loader2 size={14} className="sm:w-4 sm:h-4 animate-spin" />
                <span className="text-[10px] sm:text-xs font-bold">Loading history…</span>
              </div>
            ) : txError ? (
              <div className="flex items-center gap-2 py-4 sm:py-5 text-red-500">
                <AlertCircle size={13} className="sm:w-3.5 sm:h-3.5" />
                <span className="text-[10px] sm:text-xs font-bold">{txError}</span>
              </div>
            ) : txHistory.length === 0 ? (
              <div className="py-6 sm:py-8 text-center">
                <Zap size={20} className="sm:w-6 sm:h-6 mx-auto mb-2 text-slate-200 dark:text-gray-700" />
                <p className="text-[10px] sm:text-xs font-bold text-slate-400 dark:text-gray-600">No RZC transactions yet</p>
              </div>
            ) : (
        ...
        <div className="px-3 sm:px-5 py-2.5 sm:py-3 flex items-center justify-between gap-2 border-t border-slate-100 dark:border-white/5 bg-slate-50 dark:bg-white/[0.02]">
          <div className="flex items-center gap-1 sm:gap-1.5 min-w-0 flex-1">
            <Clock size={10} className="sm:w-3 sm:h-3 text-slate-400 dark:text-gray-600 flex-shrink-0" />
            <span className="text-[9px] sm:text-[10px] text-slate-400 dark:text-gray-600 font-medium truncate">Price: ${rzcPrice} / RZC</span>
          </div>
          <span className={`text-[8px] sm:text-[9px] font-black px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-lg uppercase tracking-wider flex-shrink-0 ${isComplete
