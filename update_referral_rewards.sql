...
-- - Signup bonus: $5 worth of RZC (instead of 25 RZC)
-- - Welcome bonus: $5 worth of RZC (instead of 150 RZC)
...
-- ============================================================================
-- ✅ Referral tracking: Working (wallet_referrals table)
-- ✅ Downline/Upline: Working (getDownline, getUpline functions)
-- ✅ RZC transactions: Working (rzc_transactions table)
-- ⚠️ Signup bonus: Currently 25 RZC (needs to be $5 worth)
-- ⚠️ Welcome bonus: Currently 150 RZC (needs to be $5 worth)
-- ❌ Package purchase commission: Not implemented yet
-- ❌ Team sales commission: Not implemented yet

-- ============================================================================
-- STEP 1: Update Activation Welcome Bonus (150 RZC → $5 worth)
-- ============================================================================
-- Note: $5 worth of RZC depends on RZC price
-- If RZC = $0.10, then $5 = 50 RZC
-- If RZC = $0.05, then $5 = 100 RZC
-- We'll use 50 RZC as default (assuming $0.10 per RZC)

-- Update MiningNodes.tsx activation-only package
-- Change: rzcReward: 150 → rzcReward: 50

-- ============================================================================
-- STEP 2: Update Signup Referral Bonus (25 RZC → $5 worth)
-- ============================================================================
-- Currently: Referrer gets 25 RZC when someone signs up
-- New: Referrer gets $5 worth of RZC (50 RZC at $0.10/RZC)

-- This is handled in the application code, not database
-- Need to update the trigger or function that awards signup bonuses

-- ============================================================================
-- STEP 3: Check Current Referral Bonus Configuration
-- ============================================================================
SELECT 
  'Current Referral System Status' as report;

-- Check signup bonuses awarded
SELECT 
  'Signup Bonuses' as type,
...
-- Check referral bonuses awarded
SELECT 
  'Referral Bonuses' as type,
  COUNT(*) as total_awarded,
  SUM(amount) as total_rzc,
  AVG(amount) as avg_amount,
  MIN(amount) as min_amount,
...
-- Check activation bonuses
SELECT 
  'Activation Bonuses' as type,
  COUNT(*) as total_awarded,
  SUM(amount) as total_rzc,
  AVG(amount) as avg_amount,
  MIN(amount) as min_amount,
...
-- ============================================================================
-- STEP 4: Create Package Purchase Commission System
-- ============================================================================
...
BEGIN
  ...
  PERFORM award_rzc_tokens(
    ...
    v_commission_rzc,
    'referral_commission',
    format('10%% commission from %s purchase by referral', p_package_name),
    jsonb_build_object(
...
-- ============================================================================
-- STEP 5: Create Team Sales Commission System (1% Weekly)
-- ============================================================================
...
-- Next steps:
-- 1. Update MiningNodes.tsx to call award_package_purchase_commission()
-- 2. Update Referral.tsx UI (already done)
-- 3. Setup weekly cron job for team sales payouts
-- ============================================================================
