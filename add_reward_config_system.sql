...
CREATE TABLE IF NOT EXISTS reward_config (
  ...
  -- Constraints
  CONSTRAINT reward_config_key_format CHECK (key ~ '^[A-Z_]+$'),
...
INSERT INTO reward_config (key, value, description, category, min_value, max_value, updated_by) VALUES
  -- Signup & Activation
  ('SIGNUP_BONUS', 4.51, 'Welcome bonus on wallet creation', 'signup', 0, 1000, 'system_init'),
  ('ACTIVATION_BONUS', 15, 'Bonus for wallet activation ($15)', 'activation', 0, 1000, 'system_init'),
  
  -- Referral Bonuses
  ('REFERRAL_BONUS', 50, 'Bonus for each successful referral', 'referral', 0, 1000, 'system_init'),
  
  -- Milestone Bonuses
  ('REFERRAL_MILESTONE_10', 75, 'Bonus at 10 referrals', 'milestone', 0, 5000, 'system_init'),
  ('REFERRAL_MILESTONE_50', 125, 'Bonus at 50 referrals', 'milestone', 0, 10000, 'system_init'),
  ('REFERRAL_MILESTONE_100', 500, 'Bonus at 100 referrals', 'milestone', 0, 50000, 'system_init'),
  ('REFERRAL_MILESTONE_250', 800, 'Bonus at 250 referrals', 'milestone', 0, 100000, 'system_init'),
  ('REFERRAL_MILESTONE_500', 1500, 'Bonus at 500 referrals', 'milestone', 0, 100000, 'system_init'),
  
  -- Transaction & Login
  ('TRANSACTION_BONUS', 1, 'Small bonus per transaction', 'transaction', 0, 100, 'system_init'),
  ('DAILY_LOGIN', 1, 'Daily login bonus', 'login', 0, 100, 'system_init'),
  ...
  -- Squad Mining
  ('SQUAD_MINING_BASE_REWARD', 1, 'Base reward per squad member for mining', 'general', 0, 1000, 'system_init'),
...
BEGIN
  ...
  RAISE NOTICE '1. Update frontend services to use get_reward_amount()';
  RAISE NOTICE '2. Create admin panel for managing configs';
