...
//           ? EVM_RPC_FAILOVER[this.currentEvmChain] || [EVM_RPC_URLS[this.currentEvmChain]]
//           : EVM_RPC_FAILOVER.sepolia || [EVM_RPC_URLS.sepolia];
...
//   // Fetch EVM transfer history via Public Block Explorer APIs (Etherscan variants)
//   async getEvmTransfers(limit = 50): Promise<any[]> {
//     if (!this.evmAccount) return [];
    
//     const apiUrl = EVM_EXPLORER_APIS[this.currentEvmChain];
//     if (!apiUrl) {
//       console.warn(`[WDK/EVM] No public explorer API for chain: ${this.currentEvmChain}`);
//       return [];
//     }

//     try {
//       const address = await this.evmAccount.getAddress();
//       const res = await fetch(`${apiUrl}?module=account&action=txlist&address=${address}&startblock=0&endblock=99999999&page=1&offset=${limit}&sort=desc`);
      
//       if (!res.ok) throw new Error(`HTTP ${res.status}`);
//       const data = await res.json();
      
//       if (data.status !== '1' && data.message !== 'No transactions found') {
//         throw new Error(data.message || 'Explorer API error');
//       }

//       return data.result || [];
//     } catch (e) {
//       console.error(`[WDK/EVM] getEvmTransfers error on ${this.currentEvmChain}:`, e);
//       return [];
//     }
//   }
...
//   /**
//    * Get multiple token balances in a single call (WDK beta.8 feature)
//    */
//   async getTokenBalances(tokenAddresses: string[], decimals: number[] = []): Promise<Record<string, string>> {
//     if (!this.evmAccount) return {};
    
//     try {
//       // Use new WDK beta.8 batch feature
...
//   /**
//    * Get comprehensive portfolio balances (native + tokens)
//    */
//   async getPortfolioBalances(): Promise<{
//     native: Record<string, string>;
//     tokens: Record<string, Record<string, string>>;
//   }> {
//     const originalChain = this.currentEvmChain;
...
//         // Get Polygon tokens
//         this.switchEvmChain('polygon').then(() => 
...
//       await this.switchEvmChain(originalChain);
...
//       };
//     } catch (error) {
//       // Ensure we restore the original chain even on error
//       await this.switchEvmChain(originalChain);
//       throw error;
//     }
//   }
...
//     // Check EVM networks
//     for (const [chain, urls] of Object.entries(EVM_RPC_FAILOVER)) {
//       try {
//         await NetworkFailover.getWorkingRpc(urls);
//         health[chain] = true;
//       } catch {
//         health[chain] = false;
//       }
