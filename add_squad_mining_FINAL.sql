-- =====================================================
-- SQUAD MINING SYSTEM - Database Migration (FINAL)
-- =====================================================
-- This adds squad mining functionality to the referral system
-- FIXED: Corrected wallet_rzc_transactions schema (no wallet_address column)
-- FIXED: Changed type from 'earn' to 'squad_mining'
...
BEGIN
  ...
  -- Create RZC transaction record (NO wallet_address column!)
...
DROP POLICY IF EXISTS "Users can view their own squad claims" ON wallet_squad_claims;
CREATE POLICY "Users can view their own squad claims"
  ON wallet_squad_claims FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own squad claims" ON wallet_squad_claims;
...
-- Verify wallet_rzc_transactions schema (should NOT have wallet_address)
SELECT column_name, data_type 
FROM information_schema.columns 
...
-- ✅ wallet_rzc_transactions: Removed wallet_address (doesn't exist)
-- ✅ wallet_rzc_transactions: Changed type to 'squad_mining'
-- ✅ wallet_notifications: Uses 'data' column (correct)
-- ✅ All user_id references use UUID type
